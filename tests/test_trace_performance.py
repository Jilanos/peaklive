...
def test_replay_progress_advances_with_the_consumed_source(qtbot, tmp_path):
    ...
    worker.records_received.connect(lambda _: worker.batch_rendered())
...
def test_a_stopped_replay_stops_within_a_bounded_number_of_batches(qtbot, tmp_path):
    ...
    worker.records_received.connect(on_batch)
    with qtbot.waitSignal(worker.finished, timeout=120_000):
        worker.start()
...
def test_a_large_load_leaves_every_retained_store_inside_its_bound(qtbot, tmp_path, monkeypatch):
    ...
    assert len(window._frames) == min(frames, window._frames.capacity)
